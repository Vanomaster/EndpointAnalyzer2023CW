...
/// <inheritdoc />
public class TrustHardPageQuery : DbQueryBase<PageModel, List<TrustedHardware>>
{
    ...
    public TrustHardPageQuery(IDbContextFactory<Context> contextFactory)
        : base(contextFactory)
    {
    }

    /// <inheritdoc/>
    protected override async Task<QueryResult<List<TrustedHardware>>> ExecuteCoreAsync(PageModel pageModel)
    {
        ...
        var entities = entitiesToFetch
            ...
            .SelectMany(benchmark => benchmark.TrustedHardware);
        ...
        if (pageModel.SearchPhrase is not null)
        {
            ...
            entities = entities.Where(entity =>
                searchWords.All(word =>
                    (entity.Name + " " + entity.HardwareId).ToLower().Contains(word.ToLower())));
        }
