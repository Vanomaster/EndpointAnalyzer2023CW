    ...
    where TClient : ClientBase
{
    ...
    public Task<QueryResult<List<string>>> GetAllNamesAsync();

    public Task<CommandResult> AddAsync(TModel model);

    public Task<CommandResult> UpdateAsync(TModel model);

    // public Task<CommandResult> AddOrUpdateAsync(TModel model);

    public Task<CommandResult> RemoveAsync(IEnumerable<TModel> model);
}